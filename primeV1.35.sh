...
Help () {
	echo "The difficulty can be set between 1 and 5 (Enter OK to contiue)"
	read cont1
	if [ $cont1 = "OK" ]
		then
			echo "Difficuly 1 will set the range at 100 (i.e. 1 to 100 (Enter OK to continue))"
			read cont2
			if [ $cont2 = "OK" ]
				then
					echo "Difficulty 2 will increase the range by 150. (i.e. 1 to 250 (Enter OK to continue))"
					read cont3
					if [ $cont3 = "OK" ]
						then
	...
	read diff
	#Timer
    echo 'Would you like to time your test (Y/n)'
    read timer
	...
	echo "Listing all Prime numbers between $st and $end (Enter OK to Continue)"
    read List
    if [ $List = "OK" ]
    then
        SECONDS=0
